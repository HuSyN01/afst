...
1.  Where does Better Auth store user sessions?
    ...
    <details>
      ...
      In the database (specifically the `session` table).
    </details>
...
3.  Why do we need to mount the auth handler to `/api/auth/**` in Hono?
    <details>
      <summary>Answer</summary>
      To allow the Better Auth client (frontend) to communicate with the backend
      for login, registration, and session management endpoints.
    </details>
