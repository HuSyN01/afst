...
<FileTree>- src - worker - db - schema.ts - index.ts</FileTree>
...
2.  Why do we use `.references(() => user.id)` in the task table?
    ...
    <details>
      <summary>Answer</summary>
      To create a Foreign Key relationship, ensuring every task belongs to a valid
      user.
    </details>
...
3.  What does `{ mode: "timestamp" }` do for integer columns?
    <details>
      <summary>Answer</summary>
      It tells Drizzle to automatically convert the stored integer (Unix timestamp)
      into a JavaScript `Date` object when reading, and vice versa when writing.
    </details>
